...
object WikipediaRanking {
   ...
   *   several seconds.
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = (for (lang <- langs) yield (lang, occurrencesOfLang(lang, rdd))).sortBy(_._2).reverse
   ...
   *   several seconds.
   */
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = index.mapValues(_.size).sortBy(_._2,false).collect().toList
